...
def compare_number_clusters_user(initial_user: str, runs: int=10):
    ...
    compare_number_clusters(number_of_clusters_list, colour="lightblue")
    compare_number_clusters(number_of_refined_clusters_list, refined=True, colour="darkblue")

def compare_number_clusters(number_of_clusters_list: list, refined=False, colour="blue"):
    ...
    ax.set_xticks(range(1, max(number_of_clusters) + 1))
    ax.set_yticks(range(0, len(number_of_clusters_list) + 1, len(number_of_clusters_list) // 5))
    ax.set_title(title)

    plt.show()

def compare_top_users_user(initial_user: str, num_clusters: int=2, num_refined_clusters: int=3, runs: int=10):
    ...
    for i, list_cluster in enumerate(list_users_prod_clusters):
        compare_top_users(list_cluster, title=f"Top 10 Users of Cluster {i} by Production Utility over {runs} runs")
    ...
    for i, list_cluster in enumerate(list_users_con_clusters):
        compare_top_users(list_cluster, title=f"Top 10 Users of Cluster {i} by Consumption Utility over {runs} runs")
    ...
    for i, list_cluster in enumerate(list_users_prod_refined_clusters):
        compare_top_users(list_cluster, title=f"Top 10 Users of Refined Cluster {i} by Production Utility over {runs} runs")
    
    l = helper(refined=True, prod=False)
    list_users_prod_refined_clusters = [[top_users_run[i] for top_users_run in l] for i in range(num_refined_clusters)]  # List of top users across runs for each cluster
    for i, list_cluster in enumerate(list_users_prod_refined_clusters):
        compare_top_users(list_cluster, title=f"Top 10 Users of Refined Cluster {i} by Consumption Utility over {runs} runs")

    

                

def compare_top_users(list_of_top_users_across_runs, title: str, n: int=10, colour="blue"):
...
if __name__ == "__main__":
    # compare_number_clusters_user("david_madras", runs=20)
    compare_top_users_user("david_madras", runs=20)
    pass
