...
Retrieved on July 14, 2023 from https://github.com/UrbanNet-Lab/LS_for_CommunityDetection_and_Clustering/blob/main
"""
...
import colorsys
from src.process.clustering.LocalSearch.LS_other_function import plot_combination
...
def hierarchical_degree_communities(G, center_num=1, auto_choose_centers=False, maximum_tree=True, seed=None, plot=True):
    '''
...
#     print("  Root Nodes: ",Potential_Center)
    ...
    degree = get_indacator_rank(root_array[:,2].astype(int))
    shortest_path = get_square(root_array[:,1].astype(int))
    ...
    multi_sort = np.array([[int(i[0]),float(i[1])] for i in multi_sort])
