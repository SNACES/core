...
class CoreDetector():
    ...
    def loop(self, user_id: str, prev_cluster, prev_wf_vector=None, default_cluster=1, v=True, skip_download=True):
        ...
        # # Refined Friends Method -> COMMENT FROM HERE
        ...
        # refined_local_neighborhood = LocalNeighbourhood(str(user_id), None, friends_map) # -> COMMENT UNTIL HERE
        social_graph = self.social_graph_constructor.construct_social_graph_from_local_neighbourhood(user_id, local_neighbourhood)  # change refined to local
        log.info("Clustering:")
