...
def run_clustering_experiment(rwf, threshold, user_count, item_count, intersection_min, popularity):
    ...
    clusters = new_algo_clustering.detect_all_communities(
        user_to_items, item_to_users, user_count, item_count, intersection_min, popularity, True)
...
def save_to_file_words(file_name, cluster_list):
    ...
    sorted_cluster_list = sorted(
        cluster_list, key=lambda cluster: cluster['count'], reverse=True)
    for cluster in sorted_cluster_list:
        ...
        cluster_info = "Core Users: {}\nCore Items: {}\nMost Typical Words: {}\nDuplicate Count: {}\n\n".format(
            core_users, core_items, most_typical_words, duplicate_count)
    ...
    for cluster in sorted_cluster_list:
        ...
        if 'shared_retweet_avg' in cluster:
            ...
            cluster_info = "Core Users: {}\nMost Typical Words: {}\nDuplicate Count: {}\nShared Retweet Average: {}\n\n".format(
                core_users, most_typical_words, duplicate_count, shared_retweet_avg)
        else:
            cluster_info = "Core Users: {}\nMost Typical Words: {}\nDuplicate Count: {}\n\n".format(
                core_users, most_typical_words, duplicate_count)
        file_object.write(cluster_info)
...
def is_satisfy_commonality(li1, li2, commonality_threshold):
    len_threshold = int(min(len(li1) * commonality_threshold, 
                            len(li2)) * commonality_threshold)
    return len(set(li1).intersection(set(li2))) >= len_threshold


def merge_recur(lists, commonality_threshold, results=None):
    # tail recursive algo
    if results is None:
        results = []
...
def merge_prev_commonality(cluster_list, x):
    merged_list = []
    for i in range(len(cluster_list)):
        cur_cluster = cluster_list[i]

        # only include clusters that are different enough from prev clusters
        is_different = True
        for prev_i in range(0, i):
            if prev_i >= 0:
                prev_cluster = cluster_list[prev_i]
                is_similar = is_satisfy_commonality(cur_cluster['items'], prev_cluster['items'], x) or\
                            is_satisfy_commonality(cur_cluster['users'], prev_cluster['users'], x)        
                if is_similar:  
                    is_different = False 
        
        if is_different:
            merged_list.append(cur_cluster)

    return merged_list


def merge_prev_count(cluster_list, x):
    merged_list = []
    for i in range(len(cluster_list)):
        cur_cluster = cluster_list[i]
        ...
        for prev_i in range(0, i):
            if prev_i >= 0:
                prev_cluster = cluster_list[prev_i]
                is_similar = len(set(cur_cluster['items']).intersection(set(prev_cluster['items']))) >= x or\
                             len(set(cur_cluster['users']).intersection(set(prev_cluster['users']))) >= x        
                if is_similar:  
                    is_different = False 
            ...
            for typicality_threshold in threshold_list:
                for user_count in top_users:
                    for item_count in top_items:
                        ...
                        file_name = "Popularity {} Intersection Min {} Threshold {}, Top Items {}, Top Users {}".format(
                            popularity, intersection_min, typicality_threshold, item_count, user_count)
                        
                        new_file_name = "Overlap {} Popularity {} Intersection Min {} Threshold {}, Top Items {}, Top Users {}".format(
                            commonality_threshold, popularity, intersection_min, typicality_threshold, item_count, user_count)
                        # new_file_name = "Common Count {} Popularity {} Intersection Min {} Threshold {}, Top Items {}, Top Users {}".format(
                        #     count, popularity, intersection_min, typicality_threshold, item_count, user_count)

                        cluster_list = get_cluster_list_from_file(file_name)

                        # merge clusters that are too similar
                        cluster_list = merge_prev_commonality(
                            cluster_list, commonality_threshold)
                        # cluster_list = merge_prev_count(
                        #     cluster_list, count)
...
user_to_interaction_rate, user_to_interaction_count = retweet_dao.user_to_interaction_rate(
    user_to_retweet, retweet_to_user)
retweet_local_neighborhood_count = retweet_dao.get_retweet_local_neighborhood_count(
    user_to_retweet)
user_to_global_interaction_rate = retweet_dao.user_to_global_interaction_rate(
    user_to_retweet, retweet_local_neighborhood_count)
user_to_relative_interaction_rate = retweet_dao.user_to_relative_interaction_rate(
    user_to_interaction_rate, user_to_global_interaction_rate)
