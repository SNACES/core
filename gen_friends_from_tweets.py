...
def evaluate_friends_list(friend_getter, user_id, friends: List[int]):
    ...
    # Number in common
    log.info(f'Number in common: {len(set(original_friends).intersection(set(friends)))}')

    # Jaccard similarity
    log.info(f'Jaccard similarity: {jaccard_similarity(original_friends, friends)}')
